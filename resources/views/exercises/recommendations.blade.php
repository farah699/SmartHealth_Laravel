{{-- filepath: c:\Users\ferie\OneDrive\Bureau\projetLaravel\SmartHealth_Laravel\resources\views\exercises\recommendations.blade.php --}}

@extends('partials.layouts.master')

@section('title', 'Recommandations IA | SmartHealth')
@section('title-sub', 'Activités Sportives')
@section('pagetitle', 'Recommandations IA')
                            ...
                            <div class="col-md-3">
                                <div class="d-flex align-items-center p-3 bg-light rounded">
                                    ...
                                    <div>
                                        ...
                                        <div class="fw-semibold">{{ $user->age ?? 28 }} ans</div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="d-flex align-items-center p-3 bg-light rounded">
                                    ...
                                    <div>
                                        ...
                                        @endphp
                                        <div class="fw-semibold {{ $imcClass }}">{{ number_format($imc, 1) }} ({{ $imcStatus }})</div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                <div class="d-flex align-items-center p-3 bg-light rounded">
                                    ...
                                    <div>
                                        ...
                                        <div class="fw-semibold">{{ $user->activities()->count() }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header bg-info text-white">
                        <h5 class="card-title mb-0">
                            <i class="fas fa-history me-2"></i>🏆 Dernières Recommandations ({{ count($recommendations) }})
                        </h5>
                    </div>
                    <div class="card-body">
                        <div class="row">
                                    ...
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-start mb-3">
                                            <h6 class="card-title">
                                                ...
                                                @endphp
                                                {{ $emoji }} {{ $rec['exercise_name'] ?? 'Exercice' }}
                                            </h6>
                                            <span class="badge bg-{{ ($rec['predicted_score'] ?? 0) >= 80 ? 'success' : (($rec['predicted_score'] ?? 0) >= 60 ? 'warning' : 'secondary') }}">
                                                {{ number_format($rec['predicted_score'] ?? 0, 1) }}%
                                            </span>
                                        </div>
                                        ...
                                        <div class="row g-2 mb-3">
                                            <div class="col-6">
                                                ...
                                                <div class="fw-semibold">{{ $rec['recommended_duration'] ?? 0 }} min</div>
                                            </div>
                                            <div class="col-6">
                                                <small class="text-muted">Difficulté</small>
                                                <div class="fw-semibold">{{ $rec['difficulty_level'] ?? 0 }}/10</div>
                                            </div>
                                        </div>
                                        
                                        <div class="mb-3">
                                            <small class="text-muted">Type:</small>
                                            <span class="badge bg-light text-dark">{{ ucfirst($rec['exercise_type'] ?? 'autre') }}</span>
                                        </div>
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            <small class="text-muted">
                                                🔥 {{ $rec['calories_per_minute'] ?? 0 }} cal/min
                                            </small>
                                            <button class="btn btn-sm btn-outline-primary start-training-btn"
                                                data-exercise-id="{{ $rec['exercise_id'] ?? $index }}"
                                                data-exercise-name="{{ $rec['exercise_name'] ?? 'Exercice' }}"
                                                data-exercise-type="{{ $rec['exercise_type'] ?? 'other' }}"
                                                data-duration="{{ $rec['recommended_duration'] ?? 30 }}"
                                                data-difficulty="{{ $rec['difficulty_level'] ?? 5 }}"
                                                data-calories="{{ $rec['calories_per_minute'] ?? 5 }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.start-training-btn')) {
            ...
            form.method = 'POST';
            form.action = '{{ route("training.start") }}';
    ...
    function generateRecommendations() {
        ...
        fetch('{{ route("exercises.recommendations.generate") }}', {
            method: 'POST',
            headers: {
